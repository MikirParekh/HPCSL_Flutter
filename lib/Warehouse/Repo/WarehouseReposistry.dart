...
class WarehouseApi {
  ...
  Future<WarehouseModel> fetchWarehouseData(BuildContext context) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['value'] != null) {
          ...
          return WarehouseModel.fromJson({'value': allResults});
        }
        throw const WarehouseException('No data found in response');
      }
      ...
      throw WarehouseException('API Error: ${response.statusCode}');

    } catch (e) {
      String errorMessage = e is WarehouseException
          ? e.message
          : 'Failed to fetch warehouse data: ${e.toString()}';
  ...
  Future<void> downloadFileforWarehouse({required String locationCode,}) async {
    try {
      ...
      final body = json.encode({
        "locationCodes": locationCode,  // Dynamic from parameter
        "customer_No": userid        // Dynamic from SharedPreferences
      });

      debugPrint('Request Body: $body');

      final url = Uri.parse('${ApiConstants.postbaseUrl}/Web_Development_Services_BondedWarehouseInventoryDownloadLocation?Company=${ApiConstants.companyName}');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData.containsKey('value')) {
          ...
          final filePath = await downloadAndSaveFile(
              fileUrl.toString(), // Ensure fileUrl is a string
  ...
  Future<String?> downloadAndSaveFile(String fileUrl, String fileName) async {
    try {
      ...
      // Handle storage permission only for Android 10 and below
  ...
  String sanitizeFileName(String invoiceNumber) {
    ...
    return 'warehouse_${sanitized}.pdf';
  }
