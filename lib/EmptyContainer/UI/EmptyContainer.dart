...
import 'package:hpcsl_1/EmptyContainer/Model/EmptyContainer_Model.dart';
...
import '../Repo/EmptyContainerReposistry.dart';

class EmptyContainer extends StatefulWidget {
  const EmptyContainer({super.key});

  @override
  State<EmptyContainer> createState() => _EmptyContainer();
}

class EmptyContainerException implements Exception {
  final String message;
  const EmptyContainerException(this.message);

  @override
  String toString() => "EmptyContainerException: $message";
}

class _EmptyContainer extends State<EmptyContainer> {
  String title = '';
  final String subtitle = "Empty Container Inventory";
  ...
  late List<EmptyContainerModel1> importDataList;

  List<EmptyContainerModel1> users = [];
  bool isLoading = true;
  final TextEditingController _controller = TextEditingController();
  final TextEditingController _BlNo = TextEditingController();
  List<EmptyContainerModel1> filteredUsers = [];
  String? userid;
  EmptyContainerApi  emptycontainer = EmptyContainerApi ();

  @override
  void initState() {
    super.initState();
    _loadDynamicTitle();
    fetchEmptyContainerData();
  }
  ...
  Future<void> fetchEmptyContainerData() async {
    userid = await StorageManager.readData('userid') as String?;
    setState(() => isLoading = true);
    try {
      final response = await emptycontainer.fetchEmptyContainerData(context);
  ...
  Widget EmptyContainerdata(BuildContext context) {
    final dataToShow =
                  ...
                  child: SizedBox(
                    ...
                    child:ElevatedButton.icon(
                      onPressed: () async {
                        ...
                        try {
                          debugPrint('Attempting to download file for invoice: S/DSI/1718/01/000011');

                          // Call the download method with the desired invoice number
                          await emptycontainer.downloadFile(
                              locationCode: user.code ?? '', // Dynamic from your data
  ...
  Widget _buildInfoRow(String title, String value) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          SizedBox(
            width: 140, // Set a fixed width for the title
            child: Text(
              title,
              style: const TextStyle(
                color: Colors.blue,
