class ViwedModel {
  String? odataContext;
  List<ViwedModel1>? value;

  ViwedModel({this.odataContext, this.value});

  ViwedModel.fromJson(Map<String, dynamic> json) {
    odataContext = json['@odata.context'];
    if (json['value'] != null) {
      value = <ViwedModel1>[];
      json['value'].forEach((v) {
        value?.add(new ViwedModel1.fromJson(v));
      });
    }
  }
...
class ViwedModel1 {
  ...
  ViwedModel1.fromJson(Map<String, dynamic> json) {
    ...
    exporter = json['Exporter'] ?? false;
    importera = json['Importera'] ?? false;
    cHA = json['CHA'] ?? false;
    freightForwarder = json['Freight_Forwarder'] ?? false;
    consignee = json['Consignee'] ?? false;
    consignor = json['Consignor'] ?? false;
    shippingLine = json['Shipping_Line'] ?? false;
    domestic = json['Domestic'] ?? false;
    consolidator = json['Consolidator'] ?? false;
  }
